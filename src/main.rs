...
impl<'a> CoqGen<'a> {
    ...
    fn header(&self) -> String {
        ...
        format!("\
            ...
            \n\
            {}\
        ", now.format("%Y-%m-%d %H:%M:%S"), self.imports(), self.preamble())
    }
...
impl<'a> TermParser<'a> {
    ...
    fn eat_op(&mut self, vars: &Vec<String>) -> Result<(), ParseError> {
        ...
        let arity = name.or_fail("didn't find op name");
        for binders in arity {
            self.eat_space()?;
            self.eat_argument(binders).or_fail("didn't find argument");
        }
        Ok(())
    }
    ...
    fn eat_name(&mut self) -> Result<Vec<u8>, ParseError> {
    ...
    fn eat_argument(&mut self, binders: u8) -> Result<(), ParseError> {
        self.eat_open().or_fail("didn't find `(`");
        let mut vars = vec![];
        for _ in 0..binders {
            // TODO: Check validity of binders
            vars.push(self.eat_var()?);
        ...
        self.eat_op(&vars).or_fail("didn't find op");
...
fn main() {
    ...
    let mut f = File::create(Path::new(coq_gen.name).with_extension("v"))
                     .expect("could not create the generated Coq file");
    write!(f, "{}", output).expect("could not write to the generated Coq file");

    // Interactive
    println!("Please enter a term to type check:");
    let mut inp = String::new();
    stdin().read_line(&mut inp).expect("Error reading input string");
    inp.pop(); // Remove \n
    let mut term_parser = TermParser { os: inp.clone(), s: inp, pos: 0, ops: &ops };
    term_parser.eat_op(&vec![]).expect("term was not valid!");
